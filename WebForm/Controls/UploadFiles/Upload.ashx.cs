...
namespace WebForm.Controls.UploadFiles
{
    ...
    public class Upload : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            var obj = FoWoSoft.Cache.IO.Opation.Get(str1 ?? "");
            ...
            {
                if (!FoWoSoft.Utility.Config.UploadFileType.Contains(Path.GetExtension(file.FileName).TrimStart('.'), StringComparison.CurrentCultureIgnoreCase))
                {
        ...
        private string getFileName(string filePath, string fileName)
        {
            ...
            {
                fileName = Path.GetFileNameWithoutExtension(fileName) + "_" + FoWoSoft.Utility.Tools.GetRandomString() + Path.GetExtension(fileName);
            }
            return fileName;
        }
        ...
        {
            DateTime date = FoWoSoft.Utility.DateTimeNew.Now;
