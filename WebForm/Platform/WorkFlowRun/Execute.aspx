...
<% 
    ...
    switch(opation)
    {
        ...
        case "completed":
            ...
            break;
        case "redirect":
            execute.ExecuteType = FoWoSoft.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Redirect;
            break;
    }
    ...
    if(stepsjson.IsArray)
    {
        foreach(LitJson.JsonData step in stepsjson)
        {
            ...
            if(id.IsGuid(out gid))
            {
                switch(execute.ExecuteType)
                {
                    ...
                    case FoWoSoft.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Back:
                        execute.Steps.Add(gid, new List<FoWoSoft.Data.Model.Users>());
                        break;
            ...
            if (execute.ExecuteType == FoWoSoft.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Redirect)
            {
    ...
    if(execute.ExecuteType == FoWoSoft.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Save ||
        execute.ExecuteType== FoWoSoft.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Completed ||
    ...
    foreach(var step in steps)
    {
        ...
        //步骤退回前事件
        if (!step.Event.BackBefore.IsNullOrEmpty() && execute.ExecuteType == FoWoSoft.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Back)
        {
    ...
    FoWoSoft.Platform.Log.Add(string.Format("处理了流程({0})", wfInstalled.Name), logContent, FoWoSoft.Platform.Log.Types.流程相关);
    ...
    foreach (var step in steps)
    {
        ...
        //步骤退回后事件
        if (!step.Event.BackAfter.IsNullOrEmpty() && execute.ExecuteType == FoWoSoft.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Back)
        {
    ...
    //归档
    if (execute.ExecuteType == FoWoSoft.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Submit 
        || execute.ExecuteType == FoWoSoft.Data.Model.WorkFlowExecute.EnumType.ExecuteType.Completed)
    {
        ...
        if (currentsteps.Count() > 0 && currentsteps.First().Archives == 1)
        {
            ...
            string archivesContents = new FoWoSoft.Platform.WorkFlowForm().GetArchivesString(formHtml, commentHtml);
