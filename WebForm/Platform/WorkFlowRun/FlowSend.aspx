...
<body>
    ...
    <%
        ...
        FoWoSoft.Data.Model.WorkFlowInstalled wfInstalled = bworkFlow.GetWorkFlowRunModel(flowid);
        if (wfInstalled == null)
        {
        ...
        if (currentStep.Behavior.FlowType == 0 && nextSteps.Count() > 0)
        {
            ...
            foreach (var step in nextSteps)
            {
                ...
                if (lines.Count() > 0)
                {
                    ...
                    #region 组织机构关系判断
                    Guid SenderID = FoWoSoft.Platform.Users.CurrentUserID;
    ...
    foreach (var step in nextSteps)
    {
        ...
        if (currentStep.Name == "发起视频会议"||
            currentStep.Name == "会场部门主管")
            selectRang = "rootid=\"" + new FoWoSoft.Platform.UsersRelation().GetMainByUserID(CurrentUser.ID).OrganizeID  + "\"";//选择范围
     
        string selectType = string.Empty;//选择类型
        var defaultMember = string.Empty;//默认处理人员

        //如果是调试模式并且当前登录人员包含在调试人员中 则默认为发起者
        if ((wfInstalled.Debug == 1 || wfInstalled.Debug == 2) && wfInstalled.DebugUsers.Exists(p => p.ID == FoWoSoft.Platform.Users.CurrentUserID))
        {
            defaultMember = FoWoSoft.Platform.Users.PREFIX + FoWoSoft.Platform.Users.CurrentUserID.ToString();
        }
        else
        {
            switch (step.Behavior.HandlerType)
            {
                ...
                case 5://发起者
                    ...
                    {
                        defaultMember = FoWoSoft.Platform.Users.PREFIX + userid.ToString();
                    }
                ...
                case 8://字段值
                    ...
                    {
                        defaultMember = new FoWoSoft.Platform.DBConnection().GetFieldValue(linkString, wfInstalled.DataBases.First().PrimaryKey, instanceid);
                    }
                    break;
                case 9://发起者主管
                    ...
                    {
                        firstSenderID = FoWoSoft.Platform.Users.CurrentUserID;
                    }
                ...
                case 10://发起者分管领导
                    ...
                    {
                        firstSenderID1 = FoWoSoft.Platform.Users.CurrentUserID;
                    }
                    ...
                    break;
                case 11://前一步处理者领导
                    defaultMember = busers.GetLeader(FoWoSoft.Platform.Users.CurrentUserID);
                    break;
                case 12://前一步处理者分管领导
                    defaultMember = busers.GetChargeLeader(FoWoSoft.Platform.Users.CurrentUserID);
                    break;
