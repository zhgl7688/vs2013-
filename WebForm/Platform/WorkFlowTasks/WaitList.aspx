...
<body>
    ...
    <table class="listtable">
        ...
        %>
            <tr>
                ...
                <td><%=task.Note %></td>
                <td><a class="viewlink" href="javascript:void(0);" onclick="detail('<%=task.FlowID %>','<%=task.GroupID %>','<%=task.ID %>');">查看</a>
                    <a class="viewlink" href="javascript:void(0);" onclick="DelTask('<%=task.FlowID %>','<%=task.GroupID %>','<%=task.ID %>');">删除</a>
                </td>
            </tr>
        <%}%>
        </tbody>
    ...
    <script type="text/javascript">
        function openTask(url, title, id) {
            $.get("/ashx/TaskFlowHandler.ashx?Model=status&id=" + id, function (result) {
                  if (result=="1") {
                    alert('该任务已处理');
                    location.reload();
                } else
        ...
        function DelTask(flowid, groupid, taskid) {
            $.get("/ashx/TaskFlowHandler.ashx?Model=DelStatus&id=" + taskid, function (result) {
                if (result == "1") {
                    alert('该任务已有处理过程，不能删除');
                  
                } else if (window.confirm('确认删除吗')) 
                {
                  
                    $.get("/ashx/TaskFlowHandler.ashx?Model=DelTask&id=" + taskid, function (result) {
