...
{
    public class WorkFlowTask : FoWoSoft.Data.Interface.IWorkFlowTask
    {
        ...
        public List<FoWoSoft.Data.Model.WorkFlowTask> GetTasks(Guid userID, out string pager, string query = "", string title = "", string flowid = "", string sender = "", string date1 = "", string date2 = "", int type = 0)
        {
            ...
            {
                sql.Append(" AND FlowID IN(" + FoWoSoft.Utility.Tools.GetSqlInString(flowid) + ")");
            }
            if (sender.IsGuid())
            {
        ...
        public List<FoWoSoft.Data.Model.WorkFlowTask> GetInstances(Guid[] flowID, Guid[] senderID, Guid[] receiveID, out string pager, string query = "", string title = "", string flowid = "", string date1 = "", string date2 = "", int status = 0)
        {
            ...
            if (senderID != null && senderID.Length > 0)
            {
                ...
                {
                    sql.Append(string.Format(" AND a.SenderID IN({0})", FoWoSoft.Utility.Tools.GetSqlInString(senderID)));
                }
            }
            if (receiveID != null && receiveID.Length > 0)
            {
                ...
                {
                    sql.Append(string.Format(" AND a.ReceiveID IN({0})", FoWoSoft.Utility.Tools.GetSqlInString(receiveID)));
                }
            }
            if (!title.IsNullOrEmpty())
            {
            ...
            {
                sql.Append(" AND a.FlowID IN(" + FoWoSoft.Utility.Tools.GetSqlInString(flowid) + ")");
            }
            if (date1.IsDateTime())
            {
