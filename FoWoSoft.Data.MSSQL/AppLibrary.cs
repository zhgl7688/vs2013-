...
{
    public class AppLibrary : FoWoSoft.Data.Interface.IAppLibrary
    {
        ...
        /// <returns></returns>
        public List<FoWoSoft.Data.Model.AppLibrary> GetPagerData(out string pager, string query = "", string order = "Type,Title", int size = 15, int number = 1, string title = "", string type = "", string address = "")
        {
            ...
            {
                WHERE.AppendFormat("AND Type IN({0}) ", FoWoSoft.Utility.Tools.GetSqlInString(type));
            }
            ...
            List<FoWoSoft.Data.Model.AppLibrary> List = DataReaderToList(dataReader);
            dataReader.Close();
            return List;
        }
        ...
        /// 查询一个类别下所有记录
        /// </summary>
        public List<FoWoSoft.Data.Model.AppLibrary> GetAllByType(string types)
        {
            string sql = "SELECT * FROM AppLibrary WHERE Type IN(" + FoWoSoft.Utility.Tools.GetSqlInString(types) + ")";
            SqlDataReader dataReader = dbHelper.GetDataReader(sql);
            List<FoWoSoft.Data.Model.AppLibrary> List = DataReaderToList(dataReader);
            dataReader.Close();
            return List;
        ...
        {
            string sql = "DELETE FROM AppLibrary WHERE ID in(" + FoWoSoft.Utility.Tools.GetSqlInString(idArray) + ")";
